...
(function( $ ){
    ...
    var defs = {
      ...
      paramsJoin      : "="
		...
		$.fn.getPagerOptions = options; // Only mentioned once. Will remove
		...
		return this.each(function(){
            ...
            $this.ctLoad( opts , ( parentClass=='perPage' ? 'First' : 'GoTo' ) );
					});
				});
			}
		});
	...
	$.fn.ctIdTable = function(){
		...
		$this.find( 'thead th' ).each(function( i ){
			$( this )
        .addClass( 'col-'+i )
        .html( '<div class="th-col-'+i+'">' + $( this ).html() + '</div>' );
	...
	$.fn.ctSelectable = function( opts ){
		...
		$this.find('td input:checkbox').click(function(){
			...
			$( this ).closest( 'tr' ).toggleClass( opts.selectableClass , checked );
		});
		...
		$this.find( 'td' ).click(function( e ){
			...
			if( opts.checkboxToggle )
				$( this ).parent( 'tr' ).find( 'td input:checkbox' ).attr( 'checked' , $( this ).parent( 'tr' ).hasClass( opts.selectableClass ) );
		});
	};
	...
	$.fn.ctCheckboxes = function(opts) {
		$this.find('th input:checkbox').click(function() {
			...
			$this.find( 'td input:checkbox' ).parent( 'td' ).parent( 'tr' ).toggleClass( opts.selectableClass , checked );
		});
	};
    ...
    $parent = $this.parent();
		...
		if( page==1 ){
			$parent.find(".ctPager img[src$='prev.gif']").attr('src', opts.images+'/prev-disabled.gif');
			$parent.find(".ctPager img[src$='first.gif']").attr('src', opts.images+'/first-disabled.gif');
			$parent.find('.ctPager').find("img[src$='next-disabled.gif']").attr('src', opts.images+'/next.gif');
			$parent.find('.ctPager').find("img[src$='last-disabled.gif']").attr('src', opts.images+'/last.gif');
		}

		if( page==pages ){
			$parent.find(".ctPager img[src$='next.gif']").attr('src', opts.images+'/next-disabled.gif');
			$parent.find(".ctPager img[src$='last.gif']").attr('src', opts.images+'/last-disabled.gif');
		}
    
    if( page!==1 ){
      if( page==pages ){
				$parent.find(".ctPager img[src$='prev-disabled.gif']").attr('src', opts.images+'/prev.gif');
				$parent.find(".ctPager img[src$='first-disabled.gif']").attr('src', opts.images+'/first.gif');
			}else{
        $parent.find('.ctPager').find("img[src$='next-disabled.gif']").attr('src', opts.images+'/next.gif');
        $parent.find('.ctPager').find("img[src$='last-disabled.gif']").attr('src', opts.images+'/last.gif');
        $parent.find(".ctPager img[src$='prev-disabled.gif']").attr('src', opts.images+'/prev.gif');
	...
	$.fn.ctLoad = function( opts , action ){
		...
		var parameters = {
      'page'      : moveToPage ,
      'sortField' : $.fn.ctPagerData.cols[sortCol] ,
      'sortOrder' : sortDir ,
      'show'      : perPage
    };
